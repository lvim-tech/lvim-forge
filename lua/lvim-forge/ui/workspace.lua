...
function M.slot()
    -- Fill the tab's editor area EXACTLY (an absolute row count, not a screen fraction) — the same geometry
    -- lvim-git's workspace uses. A `0.96 × lines` fraction overshoots by a row when `cmdheight > 0`: the
    -- surface float then runs past the global statusline, so its footer bar lands behind it and the blank
    -- "air" row above the footer becomes the visible bottom (an "extra row" + empty side margins). The
    -- available content rows are `lines` minus the tabline (1) + statusline (1) + the `cmdheight` rows; the
    -- surface centres it (its `math.max(1, …)` keeps the tabline row free), so the panel spans
    -- tabline→statusline with no gap.
...
function M.enter(view)
    ...
    -- Name the backdrop so the tabline reads e.g. "Topics" / "Notifications" instead of "[No Name]".
    pcall(api.nvim_buf_set_name, buf, (view:gsub("^%l", string.upper)))
    local win = api.nvim_get_current_win()
    pcall(api.nvim_win_set_buf, win, buf)
    -- Strip the window CHROME on the backdrop: the fill-slot surface float covers it, but its thin margin
    -- would otherwise leak the scratch buffer's line number ("1" top-left), its `~` end-of-buffer tildes, and
    -- the sign/fold columns. A clean, code-free backdrop shows nothing in that margin.
